...
def process_forecast(mode='quarterly', target_period=None, start_date=None, end_date=None):
    ...
    try:
        ...
        # For date_range mode, use date range data
            ...
            if not start_date or not end_date:
                return {"error": "Start date and end date are required for date_range mode"}, 400
            ...
            app.logger.debug(f"Fetched {len(df)} records for date_range")
        else:
    ...
    if mode == 'date_range':
        ...
        app.logger.debug(f"Generated {len(forecast_results)} date range forecasts")
        ...
        for result in forecast_results:
            ...
            incident_info = {
                ...
                "hot_months": result.get('hot_months', [])
            }
            if incident_info not in barangay_groups[bt_id]["incident_types"]:
                barangay_groups[bt_id]["incident_types"].append(incident_info)
        ...
        for barangay_data in barangay_groups.values():
            ...
            total_prediction = sum([inc.get('current_quarter_prediction', 0) for inc in sorted_incidents])
            ...
            outputs.append({
                ...
                "quarter_period": f"{start_date} to {end_date}",
                "continuous_months": f"Date Range: {start_date} to {end_date}",
                "hot_hours": barangay_data["hot_hours"],
                "incident_details": sorted_incidents,
                "date_range_prediction": total_prediction
            })
...
def home():
    return jsonify({
        ...
        "endpoints": {
            ...
            "/forecast/monthly": "Get monthly crime forecasts (optional: ?target_month=1-12)",
            "/forecast/yearly": "Get yearly crime forecasts",
            "/forecast/date_range": "Get date range crime forecasts (required: ?start_date=YYYY-MM-DD&end_date=YYYY-MM-DD)",
            "/barangays": "Get all barangays with coordinates",
            "/last_year_incidents": "Get current year's crime incidents (optional: ?quarter=1-4)",
            "/incident_types_by_month": "Get incident types by month (required: ?month=1-12&year=YYYY)",
...
@app.route('/forecast/date_range')
def forecast_date_range():
    """Get date range crime forecasts"""
    try:
        ...
        result = process_forecast(mode='date_range', start_date=start_date, end_date=end_date)
        if isinstance(result, tuple):  # Error case
            return jsonify(result[0]), result[1]
        return jsonify(result)
    except Exception as e:
        app.logger.error(f"Error in forecast_date_range: {e}")
        return jsonify({"error": str(e)}), 500
...
def get_barangays():
    ...
    try:
        ...
        conn.close()
        return jsonify({'status': 'ok', 'data': barangays})
    except Exception as e:
        app.logger.error(f"Error in get_barangays: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500


@app.route('/last_year_incidents')
def get_last_year_incidents():
    """Get current year's crime incidents with optional quarter filter"""
    try:
        quarter = request.args.get('quarter', type=int)
        conn = get_db_connection()
        incidents = fetch_last_year_incidents(conn, quarter)
        conn.close()
        return jsonify({'status': 'ok', 'data': incidents})
    except Exception as e:
        app.logger.error(f"Error in get_last_year_incidents: {e}")
        return jsonify({'status': 'error', 'message': str(e)}), 500


@app.route('/incident_types_by_month')
def get_incident_types_by_month():
    """Get incident types with barangay information for specific month and year"""
    try:
        ...
        app.logger.error(f"Error in get_incident_types_by_month: {e}")
        return jsonify({}), 500
